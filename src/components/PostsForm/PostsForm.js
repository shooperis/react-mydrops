...
const PostsForm = (props) => {
  ...
  const onSubmitFormHandler = async (event) => {
    ...
    if (
      inputDataType === "youtube" ||
      inputDataType === "vimeo" ||
      inputDataType === "soundcloud"
    ) {
    ...
    if (props.postToEditId) {
      postResponse = await fetchData({
        url: `${API_URL}/posts/${props.postToEditId}`,
        method: "PATCH",
        body: data,
        actionOrigin: "EditingPost"
      });
  ...
  return (
    <form className="post-form" onSubmit={onSubmitFormHandler} noValidate>
      ...
      <div className="right">
        <button className="button" disabled={userCtx.httpIndicator.isLoading}>Drop</button>
      </div>
    </form>
  );
};
